...
<script>
	function tacvalidcheck() {
		if ($('#taccheckbox').is(":checked")) {
			document.forms['termsandconditionsform'].submit();
		} else {
			alert("체크하셔야 회원가입가능합니다.");
		}
	}
</script>
<body>
	<div class="allwrap">
		<div id="header"></div>
		<div class="bodymargin" style="height: 614px">
			<h1>이용약관</h1>
			<form name="termsandconditionsform" action="join.jsp" method="post">
				<textarea cols="180" rows="24" style="margin-bottom:20px;">
...
- 생성정보 수집 툴을 통한 수집
				...
				<label> <input type="checkbox" class="tacchkbox" id="taccheckbox" name="taccheckbox">
				 개인정보 수집 및 이용에 동의합니다. </label> <br>
				<input type="button" class="w-btn w-btn-green" name="tacbutton" value="확인"
					onclick="tacvalidcheck()" style="margin-top:20px;">
			</form>
		</div>
		<div id="footer"></div>
	</div>
